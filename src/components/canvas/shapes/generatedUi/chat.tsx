...
const ChatWindow = ({ generatedUIId, isOpen, onClose }: Props) => {
  ...
  return (
    <div
      className={cn(
        "fixed right-5 top-1/2 transform -translate-y-1/2 w-96 h-[600px] backdrop-blur-xl bg-white/8 border-white/12 border rounded-lg z-50 transition-all duration-300 flex flex-col",
    ...
    >
      <div className="flex items-center justify-between p-4 border-b border-white/12">
        ...
        <div className="flex items-center gap-1">
          ...
          >
            <X className="w-4 h-4 hover:text-white hover:bg-white/10" />
          </Button>
        </div>
      </div>
      ...
      <div className="p-4 border-t border-white/12">
        <div className="space-y-3">
          ...
          <div className="text-xs text-white/60 text-center">
            Type your redesign request and hit enter to send
          </div>
        </div>
      </div>
    </div>
