...
Step 2a — RESPONSIVE LAYOUT (mandatory — apply to every element).
  • All grids MUST use responsive classes: grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 (never fixed column counts without mobile fallback).
  • Flex rows MUST use flex-wrap or switch to flex-col on mobile: flex flex-col sm:flex-row.
  • All images MUST have: w-full max-w-full h-auto object-cover (never fixed px widths without max-w-full).
  • All text MUST use responsive sizes: text-base sm:text-lg lg:text-xl (never fixed rem without clamp or responsive prefix).
  • All sections MUST use: w-full overflow-hidden (prevents horizontal scroll bleed).
  • Padding MUST use responsive scale: px-4 sm:px-6 lg:px-8 — never a single fixed value for all breakpoints.
  • Never use fixed width (w-[600px]) without pairing with max-w-full.
  • The outer wrapper MUST be: <div data-generated-ui class="w-full overflow-x-hidden">.
...
Step 4 — VERIFY before outputting.
  ✅ Outer wrapper has class="w-full overflow-x-hidden"
  ✅ Every grid has a grid-cols-1 mobile fallback before sm: or lg: variants
  ✅ Every flex row has flex-wrap or flex-col mobile fallback
  ✅ Every image has w-full max-w-full h-auto object-cover
  ✅ Every section has w-full overflow-hidden
  ✅ Every section has py-16 px-4 sm:px-6 lg:px-8 minimum (no bare py-8, py-12 without responsive prefix)
  ✅ Cards have p-6 minimum internal padding
  ✅ Grid gaps are gap-8 minimum
  ✅ Buttons have px-6 py-3 minimum; CTA buttons use px-8 py-4
  ✅ Only .c-* color classes used — never Tailwind color utilities
...
<output_contract>
Return ONLY the HTML wrapped in <div data-generated-ui class="w-full overflow-x-hidden">. No explanations. No markdown fences.
</output_contract>`;
...
${userMessage}
  ...
  if (currentHTML) prompt += `\n\n<current_html>\n${currentHTML}\n</current_html>`;
  if (hasWireframe) prompt += `\n\n<wireframe_context>
A wireframe image has been provided. Use it to understand the intended layout structure.
IMPORTANT: Faithfully reproduce the wireframe's visual proportions, section sizes, and element placement — the output must look obviously derived from the drawn sketch. Match column counts, hero size, sidebar presence/absence, and card grid arrangement as drawn. Do not redesign the layout; translate it as-is.
</wireframe_context>`;
...
Step 2a — RESPONSIVE LAYOUT (always re-verify even when preserving).
  • All grids MUST have grid-cols-1 sm: lg: breakpoint progression.
  • All flex rows MUST have flex-wrap or flex-col mobile fallback.
  • All images MUST have w-full max-w-full h-auto object-cover.
  • All sections MUST have w-full overflow-hidden.
  • Outer wrapper MUST be: <div data-generated-ui class="w-full overflow-x-hidden">.
  • Never use fixed pixel widths without max-w-full pairing.
...
Step 4 — VERIFY spacing, responsiveness, and color compliance.
  ✅ Outer wrapper has class="w-full overflow-x-hidden"
  ✅ Every grid has grid-cols-1 mobile-first breakpoint chain
  ✅ Every flex row has flex-wrap or flex-col sm:flex-row pattern
  ✅ Every image has w-full max-w-full h-auto object-cover
  ✅ Sections: py-16 px-4 sm:px-6 minimum with overflow-hidden
  ✅ Cards: p-6 minimum
  ✅ Buttons: px-6 py-3 minimum (CTA: px-8 py-4)
  ✅ Grid gaps: gap-8 minimum
  ✅ Colors: .c-* classes only — no Tailwind color utilities, no hex in class attributes
...
<output_contract>
Return ONLY the complete redesigned HTML wrapped in <div data-generated-ui class="w-full overflow-x-hidden">. No explanations. No markdown.
</output_contract>`;
...
Step 1 — IDENTIFY exactly which elements the request targets.
Step 2 — MODIFY only those elements. Leave all other HTML byte-for-byte identical.
Step 2a — RESPONSIVE CHECK: While modifying, ensure changed elements comply with:
  • Grids: grid-cols-1 sm: lg: chain (fix if missing)
  • Images: w-full max-w-full h-auto object-cover (fix if missing)
  • Sections: w-full overflow-hidden (fix if missing)
  • Outer wrapper: class="w-full overflow-x-hidden" (fix if missing)
Step 3 — APPLY style tokens. Do not introduce new color values.
<style_tokens>
<colors>
${formatColors(colors)}
...
Step 4 — CONFIRM output is valid HTML with no regressions and no horizontal overflow.
</task>
...
Step 1 — ANALYSE the wireframe image first.
  ...
  • Identify the layout pattern: sidebar + main, grid, stacked, etc.
  • Note every image slot and its position.
  • MATCH the wireframe's proportions as closely as possible: if the hero is ~40% of the viewport height in the sketch, make it tall; if there are 3 cards side-by-side, use a 3-col grid on desktop. The output should be immediately recognisable as the same layout that was drawn.
  • Black background = canvas only (ignore). White labels = component identifiers (do NOT render as text).
  • Do NOT add sections not shown. Do NOT omit sections that are shown.
  • If the wireframe is unclear, ambiguous, or minimal — USE CREATIVE FREEDOM to infer a complete,
    professional layout. A vague sketch is permission to design something great, not an excuse for
...
Step 1b — ANALYSE inspiration images (if provided).
  • Extract visual mood: color temperature, texture density, typographic personality, whitespace ratio.
  • Extract specific UI patterns visible: card styles, button shapes, icon usage, image aspect ratios.
  • The generated design MUST visually echo these cues — a viewer should see the resemblance without being told.
  • Map inspiration image tones to the style tokens: if inspiration is dark and moody, lean on darker token values; if light and airy, use lighter surface tokens.
...
Step 3 — BUILD the HTML structure.
  Required wrapper:
  <div data-generated-ui class="w-full overflow-x-hidden">
    <style>
      [data-generated-ui] .c-bg { background-color: #HEXVAL; }
      /* all required .c-* classes with literal hex values */
    </style>
  ...
  RESPONSIVE RULES (non-negotiable — apply to every element):
  • Grids: ALWAYS start mobile-first: grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3
    Never write grid-cols-3 alone. Always add grid-cols-1 as the base.
  • Flex rows: ALWAYS add flex-wrap or flex-col sm:flex-row pattern.
    Never write flex flex-row alone without a mobile fallback.
  • Images: ALWAYS add w-full max-w-full h-auto object-cover to every <img>.
    Never use fixed px widths (w-[400px]) without pairing max-w-full.
  • Sections: ALWAYS add w-full overflow-hidden to every <section>, <header>, <footer>, <aside>.
  • Text: Use responsive text sizes: text-base sm:text-lg lg:text-xl for body, text-2xl sm:text-3xl lg:text-4xl for headings.
  • No element may cause horizontal scroll. If unsure, add overflow-hidden to its parent.
...
Step 5 — ADD real images using Unsplash.
  ...
  • Pick the most contextually appropriate photo IDs. Vary them — don't reuse the same ID.
  • Always add crossorigin="anonymous" to every <img> tag.
  • Always add w-full max-w-full h-auto object-cover to every <img> tag.
...
Step 6 — DESIGN with shadcn/ui + Aceternity UI patterns.
  • Use shadcn/ui component patterns for all interactive elements:
    ...
    - Badges:   rounded-full px-2.5 py-0.5 text-xs font-medium
    - Alerts:   rounded-lg border p-4 with icon + message
    - Tables:   divide-y with thead/tbody and proper hover states — add w-full overflow-x-auto wrapper around all tables
  • Use Aceternity UI-inspired effects for hero/featured sections:
    - Gradient backgrounds using .c-* color classes (not Tailwind colors)
    - Subtle animated borders: ring-1 ring-white/10 (for dark themes)
    - Glass morphism cards: backdrop-blur-sm bg-white/10 (for hero overlays)
...
Step 8 — VERIFY.
  ✅ Outer wrapper has class="w-full overflow-x-hidden"
  ✅ Every section/header/footer/aside has w-full overflow-hidden
  ✅ Every grid starts with grid-cols-1 (mobile-first) before sm: lg: variants
  ✅ Every flex row has flex-wrap or flex-col sm:flex-row
  ✅ Every image has w-full max-w-full h-auto object-cover AND a real Unsplash src AND crossorigin="anonymous"
  ✅ No fixed pixel widths without max-w-full
  ✅ Tables are wrapped in overflow-x-auto
  ✅ Sections: py-16 px-4 sm:px-6 minimum (hero: py-20 or py-24)
  ✅ Cards: p-6 minimum internal padding
  ✅ Text blocks: space-y-4 minimum
  ✅ Buttons: px-6 py-3 minimum (CTA: px-8 py-4)
  ✅ Button groups: gap-4 or space-x-4
  ...
  ✅ shadcn/ui component patterns applied to all interactive elements
  ✅ At least one Aceternity-inspired effect in hero/featured section
  ✅ Layout visually matches wireframe proportions (column counts, section order, hero size)
  ✅ Visual mood matches inspiration images (color warmth, texture, typographic style)
</task>
...
INPUT PRIORITY ORDER
1) Style tokens (HIGHEST PRIORITY)
2) Wireframe structure — reproduce layout proportions faithfully
3) Inspiration images — mirror their visual mood and UI patterns
...
MATCH PROPORTIONS: column counts drawn = column counts built (on desktop).
Hero tall in sketch = hero tall in output (py-24 or more).
Sidebar present in sketch = <aside> present in output.

If wireframe unclear → infer professional structure but KEEP logical hierarchy.

STEP 1b — INSPIRATION IMAGE ANALYSIS
Extract from inspiration images:
• Color temperature and surface darkness
• Card/button shape language (sharp, rounded, pill)
• Whitespace density (airy vs dense)
• Typography personality (serif, geometric, humanist)
• Image aspect ratios and cropping style
The output must visually resemble the inspiration — not merely use similar tokens.
...
STEP 3 — RESPONSIVE RULES (NON-NEGOTIABLE)
Every grid:      grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-N
Every flex row:  flex flex-col sm:flex-row flex-wrap
Every image:     w-full max-w-full h-auto object-cover crossorigin="anonymous"
Every section:   w-full overflow-hidden
Every table:     wrapped in <div class="w-full overflow-x-auto">
Never:           fixed pixel widths without max-w-full
Never:           grid-cols-N without a grid-cols-1 base
Never:           flex-row without mobile fallback

STEP 4 — COMPONENT SYSTEM (STRICT)
...
INPUT
<input class="rounded-md border c-border px-3 py-2 c-bg c-fg focus:ring-2 focus:ring-offset-2 w-full">

BADGE
<span class="rounded-full px-2.5 py-0.5 text-xs font-medium c-accent-bg c-accent-fg">

STEP 5 — COLOR RULES (NON-NEGOTIABLE)
• ONLY .c-* classes control color.
• NEVER use Tailwind color utilities.
• NEVER use bg-[#...] or text-[#...].
• Inline style allowed ONLY for gradient or glow effects.
...
https://images.unsplash.com/photo-{ID}?w={W}&h={H}&fit=crop&auto=format
AND crossorigin="anonymous"
AND w-full max-w-full h-auto object-cover.

NO placeholders.
NO empty src.

STEP 7 — SPACING SYSTEM (STRICT)
Sections → py-16 px-4 sm:px-6 MINIMUM
Hero → py-20 or py-24
Cards → p-6 MINIMUM
Buttons → px-6 py-3 MINIMUM
CTA → px-8 py-4
...
STEP 9 — ACETERNITY EFFECT (REQUIRED)
Include ONE advanced visual effect:
• gradient text
• glass morphism card
• glow shadow
...
❌ placeholder images
❌ Tailwind colors
❌ grid-cols-N without grid-cols-1 base
❌ flex-row without mobile col fallback
❌ images without w-full max-w-full
❌ fixed widths causing overflow

OUTPUT CONTRACT
Return ONLY HTML inside <div data-generated-ui class="w-full overflow-x-hidden">.
No markdown.
No explanations.`,
  },
};
